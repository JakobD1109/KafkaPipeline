...
object SimpleKafkaConsumer extends App {
  ...
  println("Consumer 1 waiting for messages...")
  ...
  while (true) {
    ...
    for (record <- records.asScala) {
      try {
        ...
        println(s"Consumer 1 received Student: name=$name, age=$age from partition ${record.partition()}")
        
      } catch {
        case e: Exception =>
          println(s"Consumer 1 - Error: ${e.getMessage}")
          println(s"Consumer 1 - Raw bytes received: ${record.value().length} bytes from partition ${record.partition()}")
      }
    }
  }
}